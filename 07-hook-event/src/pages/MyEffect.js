...
const MyEffect = () => {
    ...
    //컴포넌트가 화면에 등장함과 동시에 1회 실행됨
    React.useEffect(() => {
        console.clear();
        console.log('[MyEffect1] %s ::: 화면에 컴포넌트가 처음 로드될 때 처리되어야 할 기능', new Date());
        window.addEventListener('resize', onMyResize);
        return () => {
            console.log("화면애서 벗어남");
            window.removeEventListener('resize', onMyResize);
        }
    }, []);

    //컴포넌트가 화면에 등장할 때와 state, props값이 변경될 때마다 매번 실행됨
    React.useEffect(() => {
        console.log('[MyEffect2] %s ::: 화면에 컴포넌트가 처음 로드되거나 state, props 중 하나라도 변경돨 경우 호출됨', new Date());
    });

    // 컴포넌트가 화면에 등장할 때와 특정 statem prorps값이 변경될 때만 실행됨
    React.useEffect(() => {
        console.log('[MyEffect3] %s ::: myBrightness 값이 변경됨', new Date());
    }, [myBrightness]);
